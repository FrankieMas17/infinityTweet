...
loadDF = loadDF.withColumnRenamed("entities.hashtags.text","hashtags")
infinityDF = sc.parallelize(loadDF.select("text").where(loadDF.hashtags.isNotNull()).where(loadDF.lang=="en").collect())
...
for line in tqdm(infinityDF.flatMap(lambda tweet : Row(tweet.text.lower().split(" "))).collect()):
    ...
    hashtags = list()
    for word in line:
        if unicode(word).startswith("#"):
            hashtags.append(word)
            #We also count the elements
            #if the dict itemsFreq already has the key corresponding to word add 1 to its count
            if itemsFreq.get(word):
                itemsFreq[word]+=1
            else: #else create the field.
                itemsFreq[word]=1
    cleanTweet = set(hashtags) #text as set of unordered words
...
dataFitems.sort(col('frequency').desc()).toPandas().to_csv('mhprovaitem_hash.csv')
dataFpairs.sort(col('frequency').desc()).toPandas().to_csv('mhprovapair_hash.csv')
